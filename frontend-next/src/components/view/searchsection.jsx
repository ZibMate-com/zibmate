  ...
  return (
    <div className=" w-full absolute -bottom-15 right-0 left-0 z-10 flex justify-center mt-4 mb-4">
      <div className="flex bg-gradient-to-br from-purple-300 via-blue-300 to-purple-300  w-max rounded-3xl shadow-blue-100 shadow-md  items-center md:gap-10 gap-20 px-4 py-5  md:text-lg text-xs font-semibold ">
        ...
        <select
          className=" p-1 font-Syne items-center hover:shadow-2xs hover:shadow-blue-300 hover:text-blue-900 rounded-4xl transition-all border-0 outline-0"
          name="looking for"
          id=""
        >
          ...
          {LookingFor.map((ele) => {
            return (
              <option className="hover:bg-blue-50  p-4 rounded-3xl " value={ele}>
                {ele}
              </option>
            );
          })}
        ...
        <span className="flex items-center gap-2">
          <input
            type="text"
            className="border bg-white border-black rounded-2xl w-sm p-2 placeholder:text-sm outline-0 text-black placeholder-black"
            name="search"
            placeholder="search for nearby pg in your locality "
          />
          <SearchIcon />
        </span>
