...
export async function POST(req: NextRequest) {
    try {
        ...
        const { pgId, roomId, fullName, email, phone, profession, aadharNumber, checkInDate, roomType, totalAmount } = await req.json();
        ...
        return NextResponse.json({ message: 'Booking requested successfully', bookingId: result.insertId }, { status: 201 });
    } catch (error) {
