...
const TicketManagementSection = () => {
  ...
  const filteredTickets = tickets.filter((t) =>
    filter === "open" ? t.status !== "closed" && t.status !== "resolved" : t.status === filter,
  );
      ...
      <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden">
        <div className="p-8 border-b border-slate-50 flex flex-col md:flex-row justify-between items-center gap-6">
          <div>
            <h2 className="text-4xl font-black text-slate-900 tracking-tight">
              Support <span className="text-orange-600">Tickets</span>
            </h2>
          ...
          <div className="flex items-center gap-3 bg-slate-50 p-1.5 rounded-2xl">
            <FilterTab
              active={filter === "open"}
              label="Open"
              count={String(tickets.filter((t) => ["open", "in_progress"].includes(t.status)).length).padStart(2, "0")}
              onClick={() => setFilter("open")}
            />
            <FilterTab
              active={filter === "resolved"}
              label="Resolved"
              count={String(tickets.filter((t) => ["resolved", "closed"].includes(t.status)).length).padStart(2, "0")}
        ...
        <div className="divide-y divide-slate-50">
          {filteredTickets.map((ticket) => (
            <div
              key={ticket.id}
              className="group hover:bg-slate-50 transition-all p-6 lg:p-8 flex flex-col lg:flex-row lg:items-center gap-6"
            >
