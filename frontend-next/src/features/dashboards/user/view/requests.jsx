...
export const SentRequests = () => {
  ...
  if (myBookings.length === 0)
    return <div className="p-10 text-center text-slate-400">You haven't sent any booking requests yet.</div>;
    ...
    <div className="flex flex-col gap-8 mt-5">
      {myBookings.map((req) => {
        return (
          <div
            key={req.id}
            className="flex text-lg items-center rounded-3xl p-6 bg-white border border-slate-100 shadow-sm hover:shadow-md transition"
          >
            ...
            <div className="flex justify-between items-center w-full ml-6">
              <div className="flex flex-col text-slate-700">
                ...
                <div className="flex gap-4 mt-3 text-xs text-slate-400 font-bold uppercase tracking-widest">
                  ...
                  <span
                    className={`px-2 py-0.5 rounded-md ${req.status === "confirmed" ? "bg-emerald-50 text-emerald-600 border border-emerald-100" : "bg-orange-50 text-orange-600 border border-orange-100"}`}
                  >
                    {req.status}
                  </span>
                </div>
