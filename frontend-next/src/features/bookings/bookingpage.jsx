...
import {
    ...
    MapPin, User, Mail, Phone, Briefcase,
            ...
            <div className="relative z-10 w-full max-w-7xl grid grid-cols-1 lg:grid-cols-12 gap-8">
                ...
                <div className="lg:col-span-8 space-y-8">
                    ...
                    <div className="bg-white/5 backdrop-blur-2xl border border-white/10 p-8 rounded-[2.5rem] flex flex-col md:flex-row justify-between items-center gap-6">
                        <div>
                            <h2 className="text-4xl font-light text-white tracking-tight">Complete <span className="font-serif italic text-orange-500">Reservation</span></h2>
                            <p className="text-stone-400 mt-1 flex items-center gap-2 text-sm">
                                Fill the form to confirm your stay
                            </p>
                        </div>
                        <div className=" p-2 rounded-2xl">
                            <h2 className="text-4xl font-light text-white tracking-tight">{bookingPropertydetails.name}</h2>
                            <p className="text-stone-400 mt-1 flex items-center gap-2 text-sm">
                                <MapPin size={14} className="text-orange-500" /> {bookingPropertydetails.address},{bookingPropertydetails.city}
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="bg-white p-8 rounded-[2.5rem] space-y-6 shadow-2xl">
                            ...
                            <InputField label="Email" icon={<Mail size={16} />} placeholder="john@example.com"
                                onChange={(v) => setBookingData({ ...bookingData, email: v })} />
                            <div className="grid grid-cols-2 gap-4">
                                <InputField label="Phone" icon={<Phone size={16} />} placeholder="+91..."
                                    onChange={(v) => setBookingData({ ...bookingData, phone: v })} />
                                <InputField label="Profession" icon={<Briefcase size={16} />} placeholder="Profession"
                                    onChange={(v) => setBookingData({ ...bookingData, profession: v })} />
                            </div>
                            <InputField label="Aadhar Number" icon={<IdCard size={16} />} placeholder="xxxxxxxxxxxx" onChange={(v) => setBookingData({ ...bookingData, aadharNumber: v })} />
                        </div>
                        ...
                        <div className="bg-white p-8 rounded-[2.5rem] space-y-6 shadow-2xl">
                            ...
                            <div className="space-y-2">
                                <label className="text-[10px] font-black uppercase tracking-widest text-stone-400 ml-1">Arrival Date</label>
                                <input type="date" className="w-full bg-stone-50 border-2 border-stone-50 rounded-2xl p-4 focus:border-orange-500 focus:bg-white outline-none transition-all" onChange={(e) => setBookingData({ ...bookingData, checkIn: e.target.value })} />

                            </div>

                            <div className="space-y-3">
                                <label className="text-[10px] font-black uppercase tracking-widest text-stone-400 ml-1">Room Preference</label>
                                <div className="grid grid-cols-1 gap-3">
                                    <RoomOption id="single" label="Single Sharing" price="5,500" active={bookingData.roomType === 'single'} onClick={() => setBookingData({ ...bookingData, roomType: 'single' })} />
                                    <RoomOption id="double" label="Double Sharing" price="4,054" active={bookingData.roomType === 'double'} onClick={() => setBookingData({ ...bookingData, roomType: 'double' })} />
                                    <RoomOption id="triple" label="Triple Sharing" price="4,054" active={bookingData.roomType === 'triple'} onClick={() => setBookingData({ ...bookingData, roomType: 'triple' })} />
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-white/5 backdrop-blur-md border border-white/10 p-6 rounded-[2rem] flex items-start gap-4">
                        <div className="p-3 bg-orange-500/20 text-orange-500 rounded-xl"><CheckCircle2 size={20} /></div>
                        <div>
                            <p className="text-white text-sm font-medium">Instant Confirmation</p>
                            <p className="text-stone-400 text-xs mt-1">Your spot is held for 20 minutes while you complete the payment. No extra hidden charges.</p>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="bg-orange-600 h-max rounded-[3rem] p-10 text-white shadow-2xl shadow-orange-900/40 relative overflow-hidden flex-1">
                        ...
                        <div className="space-y-6 mb-12">
                            ...
                            <div className="pt-6 border-t border-white/20 mt-10">
                                <p className="text-[10px] uppercase font-black text-orange-200 tracking-[0.2em] mb-2">Amount Due Today</p>
                                <div className="flex items-baseline gap-2">
                                    <span className="text-4xl font-black tracking-tighter">{bookingPropertydetails.price - bookingPropertydetails.discount}</span>
                        ...
                        <div className="space-y-4">
                            <Link href="/test">
                                <button className="w-full bg-white text-orange-600 py-6 rounded-[2rem] font-black uppercase tracking-widest text-sm flex items-center justify-center gap-3 hover:scale-[1.02] active:scale-[0.98] transition-all shadow-xl" onClick={handleBooked}>
...
const RoomOption = ({ id, label, price, active, onClick }) => (
    <button
        onClick={onClick}
        className={`flex justify-between items-center p-4 rounded-2xl border-2 transition-all ${active ? 'border-orange-500 bg-orange-50/50' : 'border-stone-100 hover:border-stone-200'
            }`}
    >
...
const Step = ({ num, active }) => (
    <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-black transition-all ${active ? 'bg-orange-500 text-white' : 'bg-white/10 text-stone-500'
        }`}>
        {num}
    </div>
);