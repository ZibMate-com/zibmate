...
export const IndividualPg = () => {
  ...
  useEffect(() => {
    async function fetchProduct() {
      ...
      try {
        ...
        const parsedOccupancy = typeof data.occupancy === "string" ? JSON.parse(data.occupancy) : data.occupancy || [];

        const parsedPrices = typeof data.prices === "string" ? JSON.parse(data.prices) : data.prices || {};

        const parsedFacilities =
          typeof data.facilities === "string" ? JSON.parse(data.facilities) : data.facilities || [];
  ...
  const lowestPrice =
    Object.values(product.prices).length > 0 ? Math.min(...Object.values(product.prices).map(Number)) : 0;

  // Format address
  const fullAddress = [product.house_number, product.street, product.landmark, product.city, product.state, product.zip]
    .filter(Boolean)
    .join(", ");
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            ...
            <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 text-right">
              ...
              {product.discount > 0 && (
                <p className="text-xs text-green-600 font-bold mt-1">Save ₹{product.discount}</p>
              )}
            </div>
          </div>
        </div>
                  ...
                  {pricingPlans.map((plan, i) => (
                    ...
                    >
                      ...
                      <div className="text-right mt-4 md:mt-0">
                        <p className="text-2xl font-black text-slate-900">₹{plan.price - product.discount}</p>
                        {product.discount > 0 && <p className="text-xs text-slate-400 line-through">₹{plan.price}</p>}
                      </div>
                    </motion.div>
                  ))}
                </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-8">
                {product.facilities.length > 0 ? (
                  product.facilities.map((item, idx) => {
                    ...
                    else if (["ac", "cooling", "air conditioning"].some((k) => itemLower.includes(k)))
                  ...
                  <div className="flex-1 text-center md:text-left">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
                      ...
                      <div className="hidden md:block bg-slate-50 p-3 rounded-xl">
                        ...
                        <p className="font-bold text-slate-800">{product.occupancy.join(", ") || "N/A"}</p>
                      </div>
                    </div>
                  </div>
                </div>
