...
export const FilterSection = ({ handlefilters, filters, handleRemoveFilters, handleSearch, searchQuery }) => {
  ...
  const isFilterActive =
    Object.values(filters).some((value) => (Array.isArray(value) ? value.length > 0 : value && value !== "")) ||
  ...
  const facilityDisplayValue =
    currentSelections.length > 0 ? `${currentSelections.length} Selected` : facilityFilter?.title;
  ...
  return (
    <div className="w-full  mx-auto py-4 px-4 sm:px-6 lg:px-8  rounded-lg shadow-md">
      ...
      <div className="flex flex-wrap items-start gap-3">
          ...
          <div className="border border-gray-300 rounded-lg p-3 bg-white w-full sm:w-auto min-w-[200px]">
            ...
            >
              <div className="flex items-center gap-2">
                ...
                />
                <span className="text-sm font-bold text-gray-700">{facilityDisplayValue}</span>
              </div>
              <ChevronDown
        ...
        {/* Other Filters (Pills) */}
        <div className="flex flex-wrap items-center gap-3">{otherFilters.map(renderFilterPill)}</div>
      </div>
    </div>
  );
};
