...
export const usePgForm = () => {
  ...
  const toggleOccupancy = (occu) => {
    setFormData((prev) => ({
      ...prev,
      occupancy: prev.occupancy.includes(occu) ? prev.occupancy.filter((f) => f !== occu) : [...prev.occupancy, occu],
    }));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.error("Failed to post property: " + (error.response?.data?.message || "Server error"));
    }
  };
