  ...
  return (
    <MotionSection className="relative min-h-[85vh] w-full px-6 md:px-12 lg:px-24 flex flex-col lg:flex-row items-center justify-between gap-12 py-16 overflow-hidden bg-[#fafafa] ">
      ...
      >
        ...
        <motion.h1
          variants={itemVariants}
          className="text-5xl md:text-6xl font-bold text-slate-900 leading-[1.15] tracking-tight"
        >
        ...
        <motion.div variants={itemVariants} className="flex flex-col gap-4 mt-8">
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm font-medium text-slate-700">
              {role == "owner"
                ? "Instant rent receipts & automated reminders"
                : "Online rent payments & instant receipts"}
            </span>
          </div>
          <div className="flex items-center gap-3">
            <div className="bg-emerald-100 p-1 rounded-full">
            ...
            <span className="text-sm font-medium text-slate-700">
              {role == "owner"
                ? " Centralized tenant & vacancy management"
                : "One-click maintenance & support requests"}
            </span>
          </div>
        </motion.div>

        <motion.div variants={itemVariants} className="flex flex-wrap items-center gap-5 mt-10">
          <Link href={role === "owner" ? "/postproperty" : "/findpg"} replace>
