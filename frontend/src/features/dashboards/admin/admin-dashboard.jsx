...
export default function AdminDashboard() {
  ...
  const { tenentRequest, ownerRequest, handleLogout, handleSendDetails } = useAdminDashboard();

  const currentRequests = activeTab === "tenant" ? tenentRequest : ownerRequest;
...
function RequestCard({ data, type, onSendDetails }) {
  ...
  return (
    <section className={`group rounded-[2.5rem] border border-slate-100 hover:border-orange-200 transition-all hover:shadow-[0_20px_50px_-20px_rgba(0,0,0,0.08)] overflow-hidden bg-white ${data.status === 'inactive' ? 'opacity-50' : ''}`}>
      <div className="p-8">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">

          <div className="flex items-start gap-6">
            <div className={`size-16 rounded-3xl flex items-center justify-center transition-all duration-500 ${type === 'owner'
            ...
            <div className="space-y-1">
              <div className="flex items-center gap-3">
                ...
                <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider ${data.status === 'inactive'
          ...
          <div className="flex-col  flex justify-center items-center gap-3 flex-wrap">
            ...
            {type === 'tenant' && (
              <button
                ...
                className={`flex-1 lg:flex-none font-bold rounded-2xl h-14 px-8 flex items-center gap-2 active:scale-95 w-full py-3 justify-center transition-all shadow-xl shadow-slate-200 ${data.status === 'inactive'
