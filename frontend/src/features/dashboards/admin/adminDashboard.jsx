...
export const AdminDashboard = () => {
    ...
    const activeComponent = useMemo(() => {
        switch (viewTab) {
            case "Properties": return <Listedproperties />;
                            ...
                            <div className="px-8 pb-8">
                                ...
                                <div className="mt-8 space-y-3">
                                    {[
                                        ...
                                        <div key={idx} className="flex items-center gap-4 p-3 rounded-2xl bg-slate-50/50 text-slate-600 text-sm font-medium border border-transparent hover:border-slate-100 transition-all">
                                ...
                                <div className="mt-8 pt-8 border-t border-slate-50 flex flex-col gap-2">
                                    <button className="flex items-center justify-between p-3 text-sm font-bold text-slate-600 hover:text-orange-600 hover:bg-orange-50 rounded-xl transition-all group">
                                        <span className="flex items-center gap-3"><KeyRound className="size-4" /> Change Password</span>
                            ...
                            <div className="flex items-center gap-1 p-3 bg-slate-50/50 border-b border-slate-100">
                                {tabs.map((tab) => (
                                    ...
                                    >
                                        ...
                                        {tab.label}
                                        {viewTab === tab.id && (
                                            <motion.div layoutId="activeTab" className="absolute bottom-2 left-1/2 -translate-x-1/2 w-1 h-1 bg-orange-600 rounded-full" />
                                        )}
                                    </button>
                                ))}
                            </div>
