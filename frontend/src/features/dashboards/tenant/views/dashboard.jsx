                    ...
                    >
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                                ...
                                <div className={`mt-4 flex items-center gap-2 ${currentStay?.status === 'confirmed' ? 'text-emerald-600 bg-emerald-50' : 'text-orange-600 bg-orange-50'} px-3 py-1 rounded-full w-fit text-xs font-bold`}>
                                    <ShieldCheck size={14} /> {currentStay?.status === 'confirmed' ? 'Stay Active' : 'Pending Confirmation'}
                                </div>
                            </div>
                                ...
                                <div className="overflow-x-auto">
                                    <table className="w-full text-left">
                                        ...
                                        <tbody className="divide-y divide-gray-50">
                                            {activeTickets.length > 0 ? activeTickets.map((t, i) => (
                                                <tr key={i} className="hover:bg-gray-50 transition-colors">
                                                    <td className="px-6 py-4 font-semibold">{t.issue}</td>
                                                    <td className="px-6 py-4 text-slate-600">{t.category}</td>
                                                    <td className="px-6 py-4">
                                                        <span className="bg-orange-100 text-orange-700 px-2 py-1 rounded-md text-[10px] font-bold uppercase">{t.status}</span>
                                                    </td>
                                                    <td className="px-6 py-4">
                                                ...
                                                <tr>
                                                    <td colSpan="4" className="px-6 py-10 text-center text-slate-400">No active maintenance tickets</td>
                                                </tr>
                                            )}
                                        </tbody>
                                    </table>
                            ...
                            <div className="space-y-6">
                                <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                                    ...
                                    <div className="flex items-center gap-4">
                                        <div className="w-12 h-12 rounded-full bg-orange-100 flex items-center justify-center text-orange-600 font-bold">Z</div>
                                        <div>
