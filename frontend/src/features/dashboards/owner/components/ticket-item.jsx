    ...
    return (
        <div className="flex items-start justify-between p-4 bg-slate-50 rounded-2xl hover:bg-orange-50 transition-colors cursor-pointer group">
            <div className="flex gap-3">
                <div className={`mt-1 size-2 roboldunded-full ${priority === 'High' ? 'bg-red-500 animate-pulse' : priority === 'Medium' ? 'bg-orange-500' : 'bg-blue-500'
                    }`} />
                <div>
                    <p className="text-sm font- text-slate-800 group-hover:text-orange-700">{issue}</p>
